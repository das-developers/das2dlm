...
pro ex05_juno_waves_survey
	...
	sQuery = string(sServer, sDataset, sBeg, sEnd, sRes, format=sFmt)
	...
	response = das2c_readhttp(sQuery)
	;das2c_datasets(response)
	...
	for i = 0, lDs.length - 1 do begin
		...
		das2c_dsinfo(dataset)  ; print info about each dataset in the request

		break

		pdTime = das2c_pdims(dataset, 'time');
		pdFreq = das2c_pdims(dataset, 'frequency');
